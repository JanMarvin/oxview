...
// rather than a hand-rolled overlay, since that class of approach (polling +
// guessing at timing against private/undocumented internals) proved fragile
// there. Known tradeoff carried over: the library's own buildPptxTextLayer
// (which this option uses internally) sizes each text span to its natural
// content width with no stretch to the next word, which can produce a visible
// gap after each word - that's the library's own minified code, not something
...
async function init() {
    ...
    const viewer = new PptxScrollViewer(container, {
        ...
        onVisibleSlideChange: (topIndex, total) => {
            ...
            targetPage = topIndex;
            pageIndicator.textContent = "Slide " + (topIndex + 1) + " / " + total;
        },
        onScaleChange: (scale) => {
    ...
    // We track the target slide ourselves rather than reading viewer.topVisibleSlide
    // fresh on every click: scrollToSlide(..., {behavior:'smooth'}) animates, and
    // topVisibleSlide likely doesn't update until that animation settles - so
    // clicking "next" again before the previous scroll finished was recomputing
    // from a stale topVisibleSlide and landing on the same slide instead of
    // advancing. onVisibleSlideChange (wired at construction, above) keeps this
    ...
    document.getElementById("btn-next").addEventListener("click", () => {
        targetPage = Math.min(viewer.slideCount - 1, targetPage + 1);
        suppressSyncUntil = Date.now() + 700;
        pageIndicator.textContent = "Slide " + (targetPage + 1) + " / " + viewer.slideCount;
        viewer.scrollToSlide(targetPage, { behavior: "smooth" });
    });
    ...
    // ---- always-available debug panel (toggle button, gated behind
    // ox_view_pptx(x, debug = TRUE) rather than a separate debug mode) ----
    ...
    // debug panel only appears when ox_view_pptx(x, debug = TRUE) requested it
